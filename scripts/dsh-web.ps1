<#
  dsh-web.ps1 — DeepSeek Harness Web GUI 启动器（局域网版 + 本地 Harness 隧道）
  ...
  本脚本启动时会自动：
    1) 若 adb 设备在线，建立 adb reverse tcp:<Port> tcp:<Port>（手机「本机 Harness」直连 PC）
...
if ($devices) {
    ...
    Write-Host "[dsh-web] adb reverse tcp:$Port 已建立（手机「本机 Harness」可用）"
