<#
  dsh-adb-tunnel.ps1 — 本地 Harness 隧道保持器

  持续监控 adb 设备，自动建立并保持 `adb reverse tcp:3080 tcp:3080`：
  手机端「本机 Harness (127.0.0.1:3080)」即可直连 PC 的 harness，
  不经局域网；设备断开重连后自动恢复，无需手动重跑。
...
function Ensure-Reverse {
    ...
    if ($list -notmatch "tcp:$Port tcp:$Port") {
        ...
        Write-Host "[dsh-adb-tunnel] $(Get-Date -Format 'HH:mm:ss') 已建立 reverse tcp:$Port -> PC"
    }
    return $true
}

if ($Once) {
    if (Ensure-Reverse) { Write-Host "[dsh-adb-tunnel] OK：手机 127.0.0.1:$Port 现在指向 PC harness" }
